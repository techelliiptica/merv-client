...
public final class MervReportsIndexHtmlWriter {
    ...
    /**
     * Merges new report folders into the open dashboard without {@code location.reload()}.
     * Pairs with {@code window.__mervLiveDashboard} exposed at the end of {@code liveDashboard}.
     */

    private static String mervIndexAdvancedSearchJs() {
        try (InputStream in = MervReportsIndexHtmlWriter.class.getResourceAsStream("/merv-index-advanced-search.js")) {
            if (in == null) {
                return "";
            }
            return new String(in.readAllBytes(), StandardCharsets.UTF_8) + "\n";
        } catch (IOException e) {
            System.err.println("Could not load merv-index-advanced-search.js: " + e.getMessage());
            return "";
        }
    }

    private static String mervIndexAjaxSyncJs() {
        return """
                (function mervIndexAjaxSync(){
                  var FOLDER_SYNC_MS=5000;
                  function parseFoldersFromIndexHtml(html){
                    var m=String(html||'').match(/window\\.MERV_REPORT_FOLDERS\\s*=\\s*(\\[[\\s\\S]*?\\]);/);
                    if(!m)return null;
                    try{return JSON.parse(m[1]);}catch(e){return null;}
                  }
                  function foldersSig(arr){return (arr||[]).join('\\u0001');}
                  function reconcileSuiteGrid(html,newFolders){
                    var api=window.__mervLiveDashboard;
                    if(!api||!api.folders||typeof api.pollAll!=='function')return false;
                    if(foldersSig(api.folders)===foldersSig(newFolders))return false;
                    api.folders.length=0;
                    newFolders.forEach(function(f){api.folders.push(f);});
                    window.MERV_REPORT_FOLDERS=newFolders.slice();
                  ...
                  async function syncIndexFolders(){
                    try{
                      var r=await fetch('./index.html?cb='+Date.now(),{cache:'no-store',credentials:'same-origin'});
                      if(!r.ok)return;
                      var html=await r.text();
                      var nf=parseFoldersFromIndexHtml(html);
                      if(!nf||!Array.isArray(nf))return;
                      reconcileSuiteGrid(html,nf);
                    }catch(e){}
                  }
                  function start(){
                    if(!window.__mervLiveDashboard){setTimeout(start,200);return;}
                    setTimeout(syncIndexFolders,1500);
                    setInterval(syncIndexFolders,FOLDER_SYNC_MS);
                  }
                  window.syncIndexFolders=syncIndexFolders;
